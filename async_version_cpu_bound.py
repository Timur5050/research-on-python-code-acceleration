...
async def async_factorize_single_print(index: int, number: int) -> None:
    ...
    divisors = await factorize_single(number)
    print(f"result of task {index}: number - {number}, divisors - {divisors}")
    print("-" * 100)


async def main_sync(in_numbers: SequenceInt = numbers) -> None:
    async with asyncio.TaskGroup() as tg:
        [tg.create_task(async_factorize_single_print(index, number)) for index, number in enumerate(in_numbers)]


if __name__ == "__main__":
    start = time.perf_counter()
    asyncio.run(main_sync(numbers))
